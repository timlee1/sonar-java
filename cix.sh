...
cd its/$TEST
mvn package -Pit-$TEST -Dsonar.runtimeVersion="$SQ_VERSION" -Dmaven.test.redirectTestOutputToFile=false \
  -Dversion.enforcer.plugin=3.0.0-M1 \
  -Dorchestrator.configUrl="file://${WORKSPACE}/its/orchestrator.properties" \
  -B -e -V